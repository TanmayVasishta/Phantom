"""
HELIX CLI — headless pipeline mode for testing without the PyQt6 GUI.

Usage:
  python helix_cli.py                    # interactive mode
  python helix_cli.py --query "list my downloads"   # single query
  python helix_cli.py --skip-health      # skip health check (faster dev loop)
"""
...
def run_query(query: str, verbose: bool = True) -> dict:
    """
    Run a single query through the full HELIX pipeline.
...
def interactive_mode() -> None:
    """Run HELIX in interactive CLI mode."""
    from utils.privacy_metrics import session_metrics

    print(
        "\n"
        "=" * 50 + "\n"
        "  HELIX CLI — Privacy-First AI OS\n"
